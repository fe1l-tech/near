...
function runMigrations(database: SqlJsDatabase): void {
  ...
  if (results.length > 0) {
    ...
    for (const row of rows) {
      executed.push(row[0] as string)
    }
  }

  if (!executed.includes('0000_initial_schema')) {
    console.log('[Database] Running initial migration...')
    database.run(initialSchema)
    database.run("INSERT INTO _migrations (name) VALUES ('0000_initial_schema')")
    saveDatabase()
    console.log('[Database] Initial migration complete.')
  }

  // 0001: 添加 Claude Code 会话 ID
  if (!executed.includes('0001_add_claude_session')) {
    console.log('[Database] Running migration: 0001_add_claude_session')
    try {
      database.run('ALTER TABLE conversations ADD COLUMN claude_session_id TEXT')
    } catch { /* 列可能已存在 */ }
    try {
      database.run('ALTER TABLE conversations ADD COLUMN claude_model TEXT')
    } catch { /* 列可能已存在 */ }
    database.run("INSERT INTO _migrations (name) VALUES ('0001_add_claude_session')")
    saveDatabase()
  }
}
