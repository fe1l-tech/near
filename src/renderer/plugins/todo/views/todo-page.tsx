...
export default function TodoPage() {
  ...
  const filterTabs: Array<{ k: 'all' | 'active' | 'completed'; label: string }> = [
    { k: 'all', label: t('todo.filters.all') },
    { k: 'active', label: t('todo.filters.active') },
    { k: 'completed', label: t('todo.filters.completed') },
  ]

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-foreground">✅ {t('todo.title')}</h1>
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <span>{t('todo.completionRate')} {stats.completionRate}%</span>
      ...
      <div className="flex gap-2">
        <Input value={newTitle} onChange={(e) => setNewTitle(e.target.value)}
          ...
          placeholder={t('todo.placeholder')} className="flex-1" />
        <Input type="date" value={newDueDate} onChange={(e) => setNewDueDate(e.target.value)}
          className="w-40 shrink-0" />
        <Button onClick={handleAdd} className="gap-1 shrink-0"><Plus className="h-4 w-4" />{t('todo.add')}</Button>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative flex-1 max-w-xs">
          ...
          <Input value={searchQuery} onChange={(e) => setSearch(e.target.value)} placeholder={t('common.search')} className="pl-8 text-sm" />
        </div>
      </div>
        ...
        {filtered.length === 0 ? (
          <GlassCard>
            <div className="py-8 text-center">
              ...
              <p className="mt-1 text-xs text-muted-foreground/70">{t('todo.emptyHint')}</p>
            </div>
          </GlassCard>
        ) : (
          filtered.map((todo) => (
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="shrink-0 rounded-full px-1.5 py-0.5 text-[10px] font-medium"
                      ...
                      {t(PRIORITY_KEYS[todo.priority] ?? 'todo.priority.medium')}
                    </span>
                  </div>
                  {todo.dueDate && (
                ...
                <div className="flex items-center gap-1">
                  ...
                  {activeTimer === todo.id ? (
                    ...
                    <button onClick={() => startTimer(todo.id)} className="rounded-lg p-1.5 text-muted-foreground hover:bg-accent hover:text-primary transition-colors">
